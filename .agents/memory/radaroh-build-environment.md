...
**How to apply:** Keep `/api` and `/api/healthz` dependency-free, use `/api/readyz` for database readiness, and never expose Radar routes until startup initialization has completed.